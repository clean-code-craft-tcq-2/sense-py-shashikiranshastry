...
# Dictionary is initialized to store computed statistics
statistics_output = {}

def calculateStats(numbers):
  
  # Try block gets executed only if input list contains only numeric data and if its length is not zero
  try:
...
class EmailAlert:
  ...
  def __init__(self):
    # Value of Email Alert is flase by default.
...
class StatsAlerter:
  ...
  def __init__(self, maxThreshold, alertList):
    ...
    # This function checks if input list contains a number greater than a threshold
    # Email Alert is True and LED is turned ON if input is greater than threshold
  def checkAndAlert(self, numbers):
    # Check if any data in the input list is higher than threshold value 
